...
export function createHudStage({ width, height }) {
  ...
  const glyphCache = new Map(); let fontData = null, flightTitleFontData = null, flightSiteFontData = null, computerFontData = null, picomagFontData = null, technicalFontData = null, logoPixels = null, logoSource = null, classicPixels = null, classicSource = null, pilotsSource = null, pilotsHighlightMask = null, pilotsTint = null, pilotsSweepPaths = null, focusOverlayGeometry = null;
  ...
  function renderFlight({ elapsed, accent = COLORS.wire }) {
    ...
    shipMaterial.color.set(accent); shipMaterial.opacity = .96; root.visible = shipExitProgress < 1; root.position.x = shipExit * FLIGHT_SHIP_EXIT_DISTANCE; root.position.y = bob; root.position.z = 0; root.scale.setScalar(1.35 + Math.sin(elapsed * 2.2) * .035); updateThrust(elapsed, .62 + shipExitProgress * .38);
  ...
  const ready = Promise.all([fetch('./assets/font-data-h/330.h').then(response => { if (!response.ok) throw new Error('Could not load Reactor.'); return response.text(); }), fetch('./assets/font-data-h/031.h').then(response => { if (!response.ok) throw new Error('Could not load Bitty.'); return response.text(); }), fetch('./assets/font-data-h/078.h').then(response => { if (!response.ok) throw new Error('Could not load Computer.'); return response.text(); }), fetch('./assets/font-data-h/270.h').then(response => { if (!response.ok) throw new Error('Could not load PicoMag.'); return response.text(); }), fetch('./assets/font-data-h/129.h').then(response => { if (!response.ok) throw new Error('Could not load Emmaline.'); return response.text(); }), fetch('./assets/font-data-h/456.h').then(response => { if (!response.ok) throw new Error('Could not load ZX Eurostile.'); return response.text(); }), fetch('./assets/glyphs/legacy-glyphs.json').then(response => { if (!response.ok) throw new Error('Could not load ATASCII glyphs.'); return response.json(); })]).then(([reactor, bitty, computer, picomag, emmaline, eurostile, glyphSource]) => { fontData = new Uint8Array(128 * 8); fontData.set(parseHeaderFont(reactor)); flightTitleFontData = parseHeaderFont(emmaline); flightSiteFontData = parseHeaderFont(eurostile); technicalFontData = parseHeaderFont(bitty); computerFontData = parseHeaderFont(computer); picomagFontData = parseHeaderFont(picomag); glyphSource.glyphs.forEach(glyph => { if (glyph.system === 'ATASCII' && glyph.internalSlot) fontData.set(glyph.bitmap, Number.parseInt(glyph.internalSlot, 16) * 8); }); glyphCache.clear(); });
